...
parser.add_argument('--exp', type=str,
                    default='LA_ACMT_Ent_opt2', help='experiment_name')
...
def train(args, snapshot_path):
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            # Entropy Selection
            EMap = entropy_map(outputs_soft[args.labeled_bs:], C=2)
            #threshold = args.Ent_th - 0.15*ramps.sigmoid_rampup(iter_num, max_iterations)
            # Opt: We further find the below strict strategy yields better results in this task
